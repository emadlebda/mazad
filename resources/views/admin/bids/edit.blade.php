    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bids.update", [$bid->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('post') ? 'is-invalid' : '' }}" name="post_id" id="post_id" required>
                    @foreach($posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('post_id') ? old('post_id') : $bid->post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('post'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bid_amount') ? 'is-invalid' : '' }}" type="number" name="bid_amount" id="bid_amount" value="{{ old('bid_amount', $bid->bid_amount) }}" step="0.01" required>
